...
if (!in_array($userAccessMethod, $accessMethod)) {
    ...
    try {
        ...
        $deactivateAccount=$connection->prepare("UPDATE tblcoursecategories SET 
        tblCourseCategoryStatus = 'InActive'
        WHERE tblCourseCategoryId= :tblCourseCategoryId");
        $deactivateAccount->bindParam(":tblCourseCategoryId", $CategoryId, PDO::PARAM_INT);
